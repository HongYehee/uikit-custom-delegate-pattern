...
final class DetailView: UIView {
    ...
    // MARK: - 멤버 저장 속성 구현
    // 멤버 데이터가 바뀌면 ===> didSet(속성 감시자) 실행
    // 속성 감시자도 (저장 속성을 관찰하는) 어쨌든 메서드임
    
    var member: Member? {
        didSet {
            guard var member = member else {
                // 멤버가 없으면 (즉, 새로운 멤버를 추가할 때의 상황)
                saveButton.setTitle("SAVE", for: .normal) // 버튼을 "SAVE"로 설정
                memberIdTextField.text = "\(Member.memberNumbers)" // 타입 저장 속성의 현재 숫자 가져오기
                return
            }
